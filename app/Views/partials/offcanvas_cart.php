            ...
            <div id="cartItemsContainer">
                ...
                foreach ($cart as $id => $qty): 
                    ...
                    if ($product):
                        ...
                        $total += $subtotal;
                        $image = base_url('uploads/products/' . ($product['image'] ?: 'default-product.svg'));
                ?>
                    <div class="cart-item border-bottom border-dark border-opacity-10 py-3 d-flex align-items-center">
                        ...
                        <div class="cart-item-info flex-grow-1">
                            ...
                            <p class="text-muted small mb-1 font-serif italic">Qty: <?= $qty ?></p>
                            <span class="cart-item-price font-serif" style="font-size: 0.9rem;">Rp <?= number_format($product['price'], 0, ',', '.') ?></span>
                        </div>
                        <div class="cart-item-actions">
                             <button class="btn btn-link text-dark p-0 remove-item" data-id="<?= $id ?>"><i class="bi bi-x"></i></button>
                        </div>
                    </div>
                <?php 
                    endif;
